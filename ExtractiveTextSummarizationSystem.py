...
st.markdown("<h1 style='text-align: center; color: white;'>EXTRACTIVE BASED TEXT SUMMARIZATION SYSTEM USING SENTIMENT ANALYSIS</h1>", unsafe_allow_html=True)
...
if choice == 'News Article':
   ...
   if option == 'Entertaiment':
      url = 'https://raw.githubusercontent.com/nnffww/TextSummarizer/main/entertaiment%20_data.csv'
      df = pd.read_csv(url,encoding="latin-1")
      st.write(df.head(50))
      st.download_button("Download",
                        df.to_csv(),
                        file_name = 'EntertaimentArticle.csv',
                        mime = 'text/csv')
   if option == 'Politics':
      url = 'https://raw.githubusercontent.com/nnffww/TextSummarizer/main/politics%20_data.csv'
      df = pd.read_csv(url,encoding="latin-1")
      st.write(df.head(50))
      st.download_button("Download",
                        df.to_csv(),
                        file_name = 'PoliticsArticle.csv',
                        mime = 'text/csv')
   if option == 'Sport':
      url = 'https://raw.githubusercontent.com/nnffww/TextSummarizer/main/sport%20_data.csv'
      df = pd.read_csv(url,encoding="latin-1")
      st.write(df.head(50))
      st.download_button("Download",
                        df.to_csv(),
                        file_name = 'SportArticle.csv',
                        mime = 'text/csv')
   if option == 'Technology':
      url = 'https://raw.githubusercontent.com/nnffww/TextSummarizer/main/technology%20_data.csv'
      df = pd.read_csv(url,encoding="latin-1")
      st.write(df.head(50))
      st.download_button("Download",
                        df.to_csv(),
                        file_name = 'TechnologyArticle.csv',
                        mime = 'text/csv')
